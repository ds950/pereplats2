...
public class Add6Controller {
    ...
    @RequestMapping(value = "/getAdd6File.docx", method = RequestMethod.GET)
    @ResponseBody
    FileSystemResource createAdd6(HttpServletRequest request, @AuthenticationPrincipal User user, Integer id, Adds adds, HttpServletResponse response) throws IOException {
        String filepath = "./src/main/resources/doc_templates/add_6.docx";
        String outpath = "C:\\Test6.docx";
        ...
        for (XWPFParagraph p : doc.getParagraphs()) {
            ...
            for (XWPFRun r : p.getRuns()) {
                ...
                if (text != null && text.contains("$")) {
                    ...
                    text = text.replace("$9", addsList.get(0).getAdd9summrub());
                    r.setText(text, 0);
                }
        ...
        //фамилия из кириллицы в латиницу
        String file_fio = cyr2lat(pensionerList.get(0).getFio());
        response.setContentType("application/vnd.ms-word");
        response.setHeader("Content-Disposition", "attachment; filename=Izveschenie_" + file_fio + "_o_facte_pereplati.docx");

        String clientIP = request.getRemoteAddr();
        Logs logs;
        String username = user.getUsername();
        String description =  "Печать / Извещение пенсионеру об установленном факте переплаты " + snils + "/ ADD6 / " +clientIP;
